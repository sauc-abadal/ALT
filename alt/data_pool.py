...
class ALT_RM_DataPool():
    # Quark And ALT_RM DataPool
...
class ALT_LMU_DataPool():
    ...
    def get_samples(self, 
                    ...
                    max_tokens: Optional[int]=64,
                    possible_scores: Optional[List[int]]=None) -> List[Dict[str, List[str]]]:
        ...
        for prompt in self.datapool.keys():
            ...
            # subsample the number of generations for each prompt  
                ...
                for score in possible_scores:
                    num_samples_to_draw[score] = num_samples_per_prompt // num_possible_scores  
                still_to_draw = num_samples_per_prompt % num_possible_scores

                for score in possible_scores:

                    sublist_indices = [i for i, x in enumerate(scores) if int(x) == score]
                    
                    sublist_generations = [generations[i] for i in sublist_indices]
                    sublist_feedbacks = [feedbacks[i] for i in sublist_indices]
                 
                    num_elem_to_keep = min(num_samples_to_draw[score], len(sublist_generations))
                    if num_elem_to_keep == 0:
                        still_to_draw += num_samples_to_draw[score]
                        continue
                    ...
                    still_to_draw += num_samples_to_draw[score] - len(sublist_generations)
