...
from alt.models.reward import GPTRewardModel
...
if __name__ == "__main__":
    ...
    # this works but it seems to be loading an untrained model... -> solved by downgrading transformers to v.4.27.1
    ...
    chosen_list = []
    reject_list = []
    with open("output/reward_model_CarperAI_test_accuracy.txt", "w") as f:
        with torch.no_grad():
            for step, batch in tqdm(enumerate(dev_dataloader), total=len(dev_dataloader)):
                ...
                if step == 0:
                    print(outputs)
                    print(f"step {step}:")
                    print(f"chosen_logits: {outputs['chosen_end_scores']}")
                    print(f"reject_logits: {outputs['rejected_end_scores']}")
                    print("........")

        print("Total accuracy: ", correct / len(dev_dataset))
        f.write(f"Total accuracy: {correct / len(dev_dataset)}")