...
input_prompts_file_valid="/cluster/work/sachan/NLF/quarkToNLF_v2/output_iter_${iteration}/NLF_conditioned_prompts_valid_varied_feedbacks.json"
...
train=$(sbatch tasks/summarization/training/bash_scripts/NLF/qNLF_to_NLF_train_start_run_2gpus.sh \
    "$accelerate_config" "$config" "$iteration" "$input_sampling_file_train" "$model_path" | awk '{print $4}')
