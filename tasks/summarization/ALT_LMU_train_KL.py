...
from alt.training_dataset_and_collator import ALT_LMU_TrainingDataset, TrainingSequenceCollatorWithPadding
from alt.data_pool import ALT_LMU_DataPool
from alt.utils.state import load_state, save_state
from alt.trainer.alt_trainer import ALTTrainer_KL
...
def main():
    ...
    possible_scores = args['train']['possible_scores']
    num_possible_scores = args['train']['num_possible_scores']

    accelerator = Accelerator(log_with="wandb", step_scheduler_with_optimizer=False)
    accelerator.print("############### ALT_LMU_train_noKL.py ###############")
    ...
    data_pool = ALT_LMU_DataPool(tokenizer=tokenizer)
    ...
    training_dataset = ALT_LMU_TrainingDataset(
        datapool=data_pool, 
        tokenizer=tokenizer,
