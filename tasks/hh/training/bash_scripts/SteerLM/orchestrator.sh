...
# train=$(sbatch --dependency=afterok:$feedback_t tasks/hh/training/bash_scripts/SteerLM/NLF_train_start_run_2gpus.sh \
#     "$accelerate_config" "$config" "$iteration" "$input_sampling_file_train" "$model_path" | awk '{print $4}')

train=$(sbatch tasks/hh/training/bash_scripts/SteerLM/NLF_train_start_run_2gpus.sh \
    "$accelerate_config" "$config" "$iteration" "$input_sampling_file_train" "$model_path" | awk '{print $4}')
